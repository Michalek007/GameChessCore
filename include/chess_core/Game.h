...
public:
    explicit Game(std::vector<std::vector<std::shared_ptr<Square>>>* board=Game::init(), Color turn=Color::white);
    static std::vector<std::vector<std::shared_ptr<Square>>>* init();
    [[nodiscard]] Color get_turn() const {return _turn;}
    void change_turn();
    std::shared_ptr<Square> get_square(Coord& coord) const;
    std::shared_ptr<Chessman> get_piece(Coord& coord) const;
    ...
    void undo_last_move();
    [[nodiscard]] std::shared_ptr<Chessman> decode_piece(std::string piece) const;
